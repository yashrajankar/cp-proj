...
// Rate limiting configuration - removed as per user request
// Login rate limiting has been disabled by removing the middleware from the login endpoints

// CSRF protection middleware with proper configuration
// For development, we'll disable CSRF protection to avoid issues
// In production, uncomment the following line and comment out the null middleware
...
// Temporary null middleware for development
...
// CSRF token endpoint - no CSRF protection needed for this endpoint
...
// Apply CSRF protection to student creation endpoint
app.post('/api/students', csrfProtection,
  ...
  body('section').notEmpty().withMessage('Section is required')
    .isIn(['B1', 'B2', 'B3']).withMessage('Section must be B1, B2, or B3'),
...
app.get('/api/results', async (req, res) => {
    try {
        const { search, examCode, section } = req.query;
        const results = await databaseService.getResults(search, examCode, section);
        res.json(results);
    } catch (error) {
...
// Apply CSRF protection to result creation endpoint
app.post('/api/results', csrfProtection,
  body('studentId').notEmpty().withMessage('Student ID is required'),
  body('examCode').notEmpty().withMessage('Exam code is required')
    .isLength({ min: 1, max: 20 }).withMessage('Exam code must be between 1 and 20 characters'),
  body('subject').notEmpty().withMessage('Subject is required')
    .isLength({ min: 1, max: 100 }).withMessage('Subject must be between 1 and 100 characters'),
  body('totalMarks').notEmpty().withMessage('Total marks is required').isFloat({ min: 1 }).withMessage('Total marks must be a positive number'),
  body('marksObtained').optional().isFloat({ min: 0 }).withMessage('Marks obtained must be a non-negative number'),
  body('grade').optional().isLength({ max: 5 }).withMessage('Grade must be less than 5 characters'),
  async (req, res) => {
    try {
        // Check for validation errors
        const errors = validationResult(req);
        ...
        if (result.marksObtained !== undefined && result.marksObtained !== null) {
            result.marksObtained = parseFloat(result.marksObtained);
        }
        result.totalMarks = parseFloat(result.totalMarks);
        
        await databaseService.createResult(result);
        
        console.log(`Created result for student ${result.studentId} in exam ${result.examCode}`);
...
app.put('/api/results/:id', async (req, res) => {
  try {
    const result = await databaseService.getResultById(req.params.id);
    ...
    // Convert marks to numbers
    if (updatedResult.marksObtained !== undefined && updatedResult.marksObtained !== null) {
      updatedResult.marksObtained = parseFloat(updatedResult.marksObtained);
    }
    if (updatedResult.totalMarks !== undefined) {
      updatedResult.totalMarks = parseFloat(updatedResult.totalMarks);
    }
    
    await databaseService.updateResult(req.params.id, updatedResult);
...
app.delete('/api/results/:id', async (req, res) => {
  try {
    ...
    await databaseService.deleteResult(req.params.id);
...
app.delete('/api/results', async (req, res) => {
  try {
    await databaseService.clearResults();
...
app.get('/api/attendance', async (req, res) => {
    try {
        const { search, date, subject, section } = req.query;
        const attendance = await databaseService.getAttendance(search, date, subject, section);
        res.json(attendance);
    } catch (error) {
...
app.get('/api/attendance/:id', async (req, res) => {
    try {
        const attendance = await databaseService.getAttendanceById(req.params.id);
...
// Apply CSRF protection to attendance creation endpoint
app.post('/api/attendance', csrfProtection,
  ...
  async (req, res) => {
    try {
        ...
        console.log(`Created attendance record for student ${attendance.studentId} on ${attendance.date}`);
...
app.put('/api/attendance/:id', async (req, res) => {
  try {
    ...
    await databaseService.updateAttendance(req.params.id, updatedAttendance);
...
app.delete('/api/attendance/:id', async (req, res) => {
  try {
    ...
    await databaseService.deleteAttendance(req.params.id);
...
app.delete('/api/attendance', async (req, res) => {
  try {
    await databaseService.clearAttendance();
    res.json({ message: 'All attendance records cleared successfully' });
  } catch (error) {
...
app.get('/api/reports/dashboard', async (req, res) => {
  try {
    ...
    const reportData = await databaseService.getDashboardReport(startDate, endDate);
    res.json(reportData);
  } catch (error) {
    console.error('Error fetching dashboard report:', error);
    res.status(500).json({ error: 'Failed to fetch dashboard report', details: error.message });
  }
});
...
app.get('/api/reports/export', async (req, res) => {
  try {
    ...
    csv += `Total Students,${reportData.totalStudents || 0}\n`;
    csv += `Total Exams,${reportData.totalExams || 0}\n`;
    csv += `Average Attendance,${reportData.avgAttendance || 0}%\n`;
    csv += `Pass Rate,${reportData.passRate || 0}%\n`;
...
// Sync students (same as bulk import) - complete the function
...
// User authentication endpoint - rate limiting disabled
...
app.get('/api/timetables', async (req, res) => {
    try {
        ...
        const timetables = await databaseService.getTimetables(search);
        res.json(timetables);
    } catch (error) {
...
// Apply CSRF protection to timetable creation endpoint
app.post('/api/timetables', csrfProtection,
  ...
  body('status').notEmpty().withMessage('Status is required')
    .isIn(['scheduled', 'completed', 'postponed', 'cancelled']).withMessage('Status must be valid'),
  async (req, res) => {
    try {
        // Check for validation errors
        const errors = validationResult(req);
...
// Apply CSRF protection to room creation endpoint
app.post('/api/rooms', csrfProtection,
  body('number').notEmpty().withMessage('Room number is required')
    .isLength({ min: 1, max: 20 }).withMessage('Room number must be between 1 and 20 characters'),
  body('building').notEmpty().withMessage('Building is required')
    .isLength({ min: 1, max: 100 }).withMessage('Building must be between 1 and 100 characters'),
...
// Apply CSRF protection to staff creation endpoint
app.post('/api/staff', csrfProtection,
  body('name').notEmpty().withMessage('Name is required')
    .isLength({ min: 1, max: 100 }).withMessage('Name must be between 1 and 100 characters'),
  ...
  body('email').optional().isEmail().withMessage('Email must be valid'),
  body('phone').optional().isMobilePhone().withMessage('Phone number must be valid'),
  body('availability').optional().isIn(['Yes', 'No']).withMessage('Availability must be Yes or No'),
  async (req, res) => {
    try {
        // Check for validation errors
        const errors = validationResult(req);
...
// Apply CSRF protection to notification creation endpoint
app.post('/api/notifications', csrfProtection,
  body('title').notEmpty().withMessage('Title is required')
    .isLength({ min: 1, max: 100 }).withMessage('Title must be between 1 and 100 characters'),
  body('message').notEmpty().withMessage('Message is required'),
  body('type').optional().isIn(['info', 'success', 'warning', 'error']).withMessage('Type must be valid'),
...
// Admin authentication endpoint - rate limiting disabled
...
app.post('/api/seatingPlans', async (req, res) => {
  try {
    ...
    // Ensure examCode is properly formatted and not too long
    if (seatingPlan.examCode) {
      // If examCode is too long, truncate it to fit in VARCHAR(20)
      if (seatingPlan.examCode.length > 20) {
        // Try to preserve the SHUF_ prefix and use a short timestamp
        if (seatingPlan.examCode.startsWith('SHUF_') || seatingPlan.examCode.startsWith('SHUFFLE_')) {
          const timestamp = Date.now().toString().slice(-4);
          seatingPlan.examCode = `SHUF_${timestamp}`;
        } else {
          // For other formats, just truncate to 20 characters
          seatingPlan.examCode = seatingPlan.examCode.substring(0, 20);
        }
      }
    }
...
app.put('/api/notifications/:id', async (req, res) => {
  try {
    ...
    await databaseService.updateNotification(req.params.id, updatedNotification);
...
app.get('/api/staff', async (req, res) => {
  try {
    const { search } = req.query;
    const staff = await databaseService.getStaff(search);
    res.json(staff);
  } catch (error) {
...
app.get('/api/rooms', async (req, res) => {
  try {
    const { search } = req.query;
    const rooms = await databaseService.getRooms(search);
    res.json(rooms);
  } catch (error) {
...
app.post('/api/staffAllocations', async (req, res) => {
  try {
    ...
    if (existingAllocation) {
      // Update existing allocation instead of returning error
      const updatedAllocation = await databaseService.updateStaffAllocation(
...
// Update the roomAssignments endpoint to properly sort shuffled seating plans
app.get('/api/roomAssignments', async (req, res) => {
  try {
    ...
    const seatingPlans = await databaseService.getSeatingPlans();
    console.log(`Found ${seatingPlans.length} seating plans in database`);
    
    // Filter for shuffled plans (those with SHUF_ in the examCode)
    const shuffledPlans = seatingPlans.filter(plan => 
      plan.examCode && plan.examCode.startsWith('SHUF_')
    );
    ...
    if (shuffledPlans.length > 0) {
      ...
      // Sort shuffled plans by their creation order (examCode contains timestamp)
      const sortedShuffledPlans = [...shuffledPlans].sort((a, b) => {
        // Extract timestamp from examCode (SHUF_timestamp)
        const timestampA = a.examCode.split('_')[1] || '0';
        const timestampB = b.examCode.split('_')[1] || '0';
        return parseInt(timestampB) - parseInt(timestampA); // Most recent first
      });
      
      // Use the most recent shuffled plan
      const latestShuffledPlan = sortedShuffledPlans[0];
      console.log(`Using latest shuffled plan with examCode: ${latestShuffledPlan.examCode}`);
      
      // Convert seating plans to room assignments format
      const assignments = sortedShuffledPlans.map(plan => {
        // Extract students from seats
        const students = [];
        if (plan.planData && plan.planData.seats && Array.isArray(plan.planData.seats)) {
          plan.planData.seats.forEach(seat => {
        ...
        // Get room information from planData or from plan directly
        const roomInfo = plan.planData || {};
        
        return {
          _id: plan.planData.roomId || plan.roomId || plan._id,
          number: plan.planData.roomNumber || plan.roomNumber || roomInfo.roomNumber || 'N/A',
          building: plan.planData.building || plan.building || roomInfo.building || 'N/A',
          capacity: plan.planData.capacity || plan.capacity || roomInfo.capacity || 0,
          students: students
        };
      });
      ...
      console.log('Returning shuffled room assignments:', JSON.stringify(result, null, 2));
      res.json(result);
      return;
    }
...
// Update the shuffleClassrooms endpoint to use an even shorter examCode
app.post('/api/shuffleClassrooms', async (req, res) => {
  try {
    ...
    // Log the room order before and after shuffling
    console.log('Room order before shuffling:', rooms.map(r => `${r.building}-${r.number}`));
    console.log('Room order after shuffling:', shuffledRooms.map(r => `${r.building}-${r.number}`));
    ...
    for (let roomIndex = 0; roomIndex < totalRooms; roomIndex++) {
      ...
      // Use a very short examCode to avoid database column size issues (max 20 chars)
      const timestamp = Math.floor(Date.now() / 1000).toString(); // Unix timestamp in seconds
      // Take only the last 4 digits to keep it short
      const shortTimestamp = timestamp.substring(Math.max(0, timestamp.length - 4));
      // Include room ID to make each examCode unique
      const roomSuffix = room._id.toString().slice(-2); // Last 2 digits of room ID
      const seatingPlan = {
        planData: {
          roomId: room._id,
          building: room.building,
        ...
        examCode: `SHUF_${shortTimestamp}${roomSuffix}` // This will be SHUF_XXXXYY format (11 chars total)
      };
...
function generateSmartAssignments(staff, rooms, days) {
  ...
  for (let day = 1; day <= days; day++) {
    ...
    // Assign staff to rooms (now allowing duplicates on the same day)
    ...
    for (let i = 0; i < shuffledRooms.length; i++) {
      ...
      // Select a staff member from the pool (cycling through if needed)
      const staffIndex = i % staffPool.length;
      const assignedStaffMember = staffPool[staffIndex];
...
app.get('/api/user/exams', async (req, res) => {
    try {
        // For now, we'll return all exams since there's no user authentication mechanism
        // In a real implementation, this would filter exams for the authenticated user
...
app.get('/api/user/exams/upcoming', async (req, res) => {
    try {
        // For now, we'll return all upcoming exams since there's no user authentication mechanism
        // In a real implementation, this would filter exams for the authenticated user
        const timetables = await databaseService.getTimetables();
...
app.get('/api/user/seating', async (req, res) => {
    try {
        // For now, we'll return all seating arrangements since there's no user authentication mechanism
        // In a real implementation, this would filter seating arrangements for the authenticated user
        const seatingPlans = await databaseService.getSeatingPlans();
...
app.get('/api/user/dashboard', async (req, res) => {
    try {
        ...
        // Calculate upcoming exams (next 30 days)
        const today = new Date();
        const next30Days = new Date();
        next30Days.setDate(today.getDate() + 30);
        ...
        const assignedSeats = seatingPlans.length;
        
        // Get upcoming exams list (next 5 exams)
        const upcomingExamsList = timetables
            .filter(timetable => {
                const examDate = new Date(timetable.date);
        ...
        // Get seating arrangements (first 3)
        const seatingArrangements = seatingPlans
            .map((plan, index) => {
                const planData = plan.planData || {};
                ...
                if (planData.seats && planData.seats.length > 0) {
                    ...
                    if (studentSeat) {
                        if (studentSeat.row !== undefined && studentSeat.col !== undefined) {
                            seatInfo = `${String.fromCharCode(65 + studentSeat.row)}${studentSeat.col + 1}`;
                        } else {
                            seatInfo = `Seat ${studentSeat.id || index + 1}`;
                        }
                    } else {
                        const firstSeat = planData.seats[0];
                        if (firstSeat.row !== undefined && firstSeat.col !== undefined) {
                            seatInfo = `${String.fromCharCode(65 + firstSeat.row)}${firstSeat.col + 1}`;
                        } else {
                            seatInfo = `Seat ${index + 1}`;
                        }
                    }
...
// Removed user notifications endpoint as the feature is no longer needed
...
initDB().then(() => {
  ...
  const startServer = () => {
    const server = app.listen(currentPort, () => {
      ...
      console.log(`👥 User dashboard: http://localhost:${currentPort}/user-features/dashboard/dashboard.html`);
